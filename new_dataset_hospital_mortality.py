...
dataset = Dataset(dataset_name='data01', output_id=2, input_ids=list(range(3, 10)),
                  categorical_ids=[2, 4, 6, 7, 8, 9, 10, 11, 12, 13, 14], biased_ids=[4], normalize=True)



(X_train, y_train), (X_test, y_test) = dataset.get_data(biased=True, split=TRAIN_TEST_SPLIT, random_state=SEED)
biased_ml = RandomForestClassifier(max_depth=200, random_state=SEED, n_jobs=-1)
...
unbiased_ml = RandomForestClassifier(max_depth=200, random_state=SEED, n_jobs=-1)
